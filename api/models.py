...
def _validate_detail_string_lengths(values: list[dict[str, str]], *, field: str) -> list[dict[str, str]]:
    for index, detail in enumerate(values):
        ...
        for key, item in detail.items():
            if len(str(key)) > PLAN_LIST_ITEM_MAX_CHARS or len(str(item)) > GENERIC_PROFILE_NOTES_MAX_CHARS:
                raise ValueError(
                    f"{field}[{index}] strings must be at most {GENERIC_PROFILE_NOTES_MAX_CHARS} characters long"
                )
    return values
