...
def _has_non_empty_values(value: object) -> bool:
...
def profile_activation_complete(profile: object) -> bool:
    """Return whether the persisted profile step has authoritative core data."""

    full_name = str(_value(profile, "full_name") or "").strip()
    technical_style = _value(profile, "technical_style")
    return bool(full_name and _has_non_empty_values(technical_style))
...
def reconcile_activation_xp(
    ...
    *,
    ...
    Awards are monotonic: later profile edits or plan archival never remove XP.
    """

    results: list[dict] = []
    try:
        if profile_activation_complete(profile):
            profile_result = _award(
                store,
        ...
        if latest_plan is not None and plan_activation_ready(latest_plan):
            plan_result = _award(
                store,
                athlete_id=athlete_id,
    ...
    except Exception:  # noqa: BLE001 - reconciliation must never break /api/me
        logger.exception("[xp] activation reconciliation failed athlete_id=%s", athlete_id)
    return results
