...
def _normalize_effort(value: Any) -> dict[str, Any] | None:
    ...
    if isinstance(value, dict):
        ...
        if isinstance(raw_value, str):
            ...
            numeric_text = re.sub(r"^(?:RPE|RIR)\s*", "", text, flags=re.I)
            normalized = _normalize_numeric_or_range(numeric_text)
            if isinstance(normalized, str):
                out["value"] = normalized
            elif re.fullmatch(r"(?:(?:RPE|RIR)\s*)?[\d.\s–-]+", text, re.I):
                # Numeric-looking malformed and reversed ranges are not cues.
                return None
    ...
    if isinstance(value, str):
        ...
        if match is None:
            return None
        method = "RIR" if (match.group(1) or "").upper() == "RIR" else "RPE"
