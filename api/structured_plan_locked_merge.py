...
_TACTICAL_WATCH_SPEC = _LOCKED_CONTENT_SPECS[0]


def _open_plan_watch_rotation(planning_brief: Any) -> list[Mapping[str, Any]]:
    """The open plan's weekly Tactical Watch rotation, or an empty list."""
    if not isinstance(planning_brief, Mapping):
        return []
    spec = planning_brief.get("open_plan_spec")
    if not isinstance(spec, Mapping) or spec.get("plan_type") != _OPEN_PLAN_TYPE:
        return []
    watch_spec = spec.get("tactical_watch")
    if not isinstance(watch_spec, Mapping):
        return []
    rotation = watch_spec.get("weekly_rotation")
    if not isinstance(rotation, (list, tuple)):
        return []
    return [entry for entry in rotation if isinstance(entry, Mapping)]


def _open_plan_week_entry(
    rotation: list[Mapping[str, Any]], week_index: int, single_template_week: bool
) -> Mapping[str, Any] | None:
    """The rotation entry that owns ``week_index``.

    A one-week card is the reusable template for the whole block, so it carries
    the Week 1 watch. A four-week card matches each week to its own entry, and a
    week the rotation does not cover gets nothing rather than a repeat.
    """
    if not rotation:
        return None
    if single_template_week:
        week_index = 1
    for entry in rotation:
        if int(entry.get("week") or 0) == week_index:
            return entry
    return None


def merge_open_plan_tactical_watch(
    structured_plan: dict[str, Any], planning_brief: Any
) -> LockedMergeResult:
    """Render the open plan's weekly Tactical Watch as a real structured card.

    A dated camp reaches :func:`merge_locked_structured_content` through a
    governed role on a countdown day. An open plan has neither: no countdown
    labels, no weekly_role_map, so that pathway matched nothing and the watch
    existed only as an instruction to the finalizer. When the model declined to
    write the card — the converter contract tells it to preserve the source
    session spine and invent nothing — the watch silently vanished from the
    athlete's plan.

    The rotation is server-decided content, so it is applied here the same way
    and from the same bank fields as the dated path. Placement is the week's
    first training day; the card is a zero-load skill session that sits
    alongside that day's training rather than replacing it.
    """
    plan = copy.deepcopy(structured_plan)
    result = LockedMergeResult(plan=plan)
    rotation = _open_plan_watch_rotation(planning_brief)
    if not rotation or not isinstance(plan.get("weeks"), list):
        return result

    spec = _TACTICAL_WATCH_SPEC
    session_title = _normalise(spec.session_title)
    weeks = [week for week in plan["weeks"] if isinstance(week, Mapping)]
    single_template_week = len(weeks) == 1

    for position, week in enumerate(weeks, start=1):
        week_index = int(week.get("week_index") or position)
        entry = _open_plan_week_entry(rotation, week_index, single_template_week)
        if entry is None:
            continue
        watch = entry.get("tactical_watch")
        watch = watch if isinstance(watch, Mapping) else {}
        name = str(entry.get("name") or watch.get("name") or "Tactical Watch")
        day_label = f"Week {week_index}"

        days = [day for day in week.get("days") or [] if isinstance(day, dict)]
        if not days:
            result.unresolved.append(
                LockedMergeIssue(day_label, name, "open plan week has no training day")
            )
            continue
        ...
        existing = [
            ...
            if isinstance(session, dict) and _normalise(session.get("title")) == session_title
        ]
        if len(existing) > 1:
            result.unresolved.append(
                LockedMergeIssue(
                    day_label, name, f"{spec.session_title} session not uniquely resolved"
                )
            )
            continue

        if existing:
            # The finalizer did write the card: repair it to the bank's content
            ...
            days[0].setdefault("sessions", []).append(session)
        ...
        session["title"] = spec.session_title
        session["session_type"] = spec.session_type
        session["objective"] = watch.get("why") or "Review the tactical plan."
        session["mindset_anchor"] = _mindset_anchor(watch)
        block.update(
            {
                "display_name": name,
                "duration": _duration(entry if entry.get("duration_min") else watch),
                "coaching_cues": [str(value) for value in watch.get("instructions") or []],
        ...
        if watch.get("progress"):
            block["progression_rule"] = watch["progress"]
        result.applied.append(LockedMergeApplication(day_label, name))
...
def merge_planner_owned_structured_content(
    ...
    # An open plan has no countdown day for the locked pathway to resolve, so
    # its weekly Tactical Watch is applied from open_plan_spec instead. This is
    # a no-op for every dated camp.
    open_plan_result = merge_open_plan_tactical_watch(result.plan, planning_brief)
    result.plan = open_plan_result.plan
    result.applied.extend(open_plan_result.applied)
    result.unresolved.extend(open_plan_result.unresolved)
