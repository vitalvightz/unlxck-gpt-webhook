...
- Resumed jobs may end as:
  ...
  - plan `review_required` / `held_for_review` / `publishable_with_flags` (job `review_required` or `completed` according to `job_status_for_plan_status`)
...
- `plan_id` must point to an existing `plans` row, or
- the plan status must be intentional `triage_blocked`.
