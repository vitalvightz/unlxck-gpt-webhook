...
- **Required functions/RPCs:** `change_profile_username`,
  ...
  `create_generation_job_with_daily_limit`, `prevent_self_role_escalation`,
  `prevent_username_policy_bypass`, `is_admin`, and
  `validate_generation_job_active_lock` (called at backend startup).
