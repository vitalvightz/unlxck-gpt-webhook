...
from .stage2_render_guards import has_active_injury_from_training_context
...
def derive_competitive_maturity(status: str, record: str) -> dict:
...
def is_high_pressure_weight_cut(*, athlete_model: dict) -> bool:
...
def build_athlete_model(
    *,
    ...
    record_profile = derive_competitive_maturity(training_context.status, record)
    plan_creation_dt = _athlete_calendar_now(
    ...
    has_active_injury = has_active_injury_from_training_context(training_context)
    athlete_model = {
        ...
        "readiness_flags": derive_readiness_flags(
            fatigue=training_context.fatigue,
            weight_cut_risk=training_context.weight_cut_risk,
            weight_cut_pct=training_context.weight_cut_pct,
            injuries=training_context.injuries,
