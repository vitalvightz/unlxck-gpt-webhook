...
def _instability_event_is_negated(text: str, synonym: str) -> bool:
    escaped = re.escape(synonym)
    return bool(
        re.search(rf"\b(?:no|not|without|denies?|denied)\s+(?:\w+\s+){{0,3}}{escaped}\b", text)
    )
    ...
    for level, synonym in hits:
        if level == "high" and synonym in _CONTEXTUAL_HIGH_TERMS:
            ...
            if synonym != "instability" and not contextual_high_allowed:
                continue
        if level == "high" and synonym in _INSTABILITY_EVENT_TERMS and _instability_event_is_negated(lowered, synonym):
            continue
...
def _surface_injury_assessment(injuries: Iterable[str | dict]) -> tuple[int, int, list[str]]:
    ...
    for injury in injuries:
        ...
        fallback_type = ""
        if isinstance(injury, dict):
            fallback_type = str(injury.get("injury_type") or "").strip().lower()
            raw_text = str(injury.get("original_phrase") or injury.get("raw") or "")
        else:
            raw_text = str(injury)
        parsed_type_found = False
        for phrase in split_injury_text(raw_text):
            if is_restriction_phrase(phrase):
                continue
