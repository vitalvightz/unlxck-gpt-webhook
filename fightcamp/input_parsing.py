...
_SWELLING_ESCALATION_CONTEXT_RE = re.compile(
    r"\b(?:rapid|rapidly|worsening|worse|severe)\s+swelling\b"
    r"|\bswelling\b[\w\s,-]{0,40}\b(?:cannot|can'?t|unable\s+to)\s+bear\s+weight\b"
    r"|\bswelling\b[\w\s,-]{0,40}\b(?:deformity|deformed)\b"
    r"|\bswelling\b[\w\s,-]{0,40}\b(?:after|from)\s+(?:tackle|impact|collision|hit|fall)\b"
