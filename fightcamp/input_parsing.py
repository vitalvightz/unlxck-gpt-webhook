...
_GUIDED_SERIOUS_TYPES = {"fracture", "dislocation", "post_surgery"}
_GUIDED_EVIDENCE_ONLY_TYPES = {"tendon_ligament", "unspecified"}
...
_GUIDED_RUPTURE_EVIDENCE = re.compile(
    r"\b(?:rupture|ruptured|avulsion|detached|complete tear|full tear|confirmed tear|"
    r"confirmed rupture|deformity|cannot bear weight|cannot walk)\b",
    re.IGNORECASE,
)
...
def _resolve_guided_injury_type(guided_injury: GuidedInjury) -> tuple[str | None, str | None]:
    ...
    surface_mapped_type = None
    if guided_type == "surface_injury":
        surface_mapped_type = _GUIDED_SURFACE_TYPE_TO_INJURY_TYPE.get(
            (guided_injury.surface_type or "").strip().lower(),
        )

    rupture_evidence = bool(_GUIDED_RUPTURE_EVIDENCE.search(combined_text))

    if parser_type:
        final_type = parser_type
    elif surface_mapped_type:
        final_type = surface_mapped_type
    elif guided_type in _GUIDED_SERIOUS_TYPES:
