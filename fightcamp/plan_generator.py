...
def parse_tally_fields(fields_list):
    """Convert Tally style field list to a flat dictionary."""
    mapping = {}
    for field in fields_list:
        ...
        if isinstance(value, list):
            if "options" in field:
                ...
                mapping[label] = ", ".join(str(v) for v in value)
        else:
            mapping[label] = str(value).strip() if value is not None else ""
    return mapping


def generate_training_plan(fields: dict):
    """Generate the fight camp plan text from a field dictionary."""
    ...
    target_weight = fields.get("Target Weight (kg)", "")
    height = fields.get("Height (cm)", "")
    fighting_style_technical = fields.get("Fighting Style (Technical)", "")
    fighting_style_tactical = fields.get("Fighting Style (Tactical)", "")
    stance = fields.get("Stance", "")
    status = fields.get("Professional Status", "")
    record = fields.get("Current Record", "")
    next_fight_date = fields.get("When is your next fight?", "")
    rounds_format = fields.get("Rounds x Minutes", "")
    frequency_raw = fields.get("Weekly Training Frequency", "")
    fatigue = fields.get("Fatigue Level", "")
    equipment_access = fields.get("Equipment Access", "")
    available_days = fields.get("Time Availability for Training", "")
    ...
    injuries = fields.get("Any injuries or areas you need to work around?", "")
    key_goals = fields.get("What are your key performance goals?", "")
    weak_areas = fields.get("Where do you feel weakest right now?", "")
    training_preference = fields.get("Do you prefer certain training styles?", "")
    mental_block = fields.get(
    ...
    target_val = (
        float(target_weight) if target_weight.replace('.', '', 1).isdigit() else 0.0
    )
    weight_cut_risk_flag = (
        weight_val - target_val >= 0.05 * target_val if target_val else False
    )
    weight_cut_pct_val = (
        round((weight_val - target_val) / target_val * 100, 1) if target_val else 0.0
    )
    ...
    training_context = {
        ...
        "training_preference": training_preference.strip().lower()
    ...
    return fight_plan_text, full_name
