...
def generate_plan_blocks(
    *,
    ...
    if "strength" in injury_safety_decision.blocked_modules:
        ...
        strength_blocks = {phase: ({"exercises": [], "why_log": [{"name": "blocked", "explanation": safety_message}]}) if context.phase_active(phase) else None for phase in PHASES}
    ...
    if "conditioning" in injury_safety_decision.blocked_modules:
        safety_message = _safety_block_message(injury_safety_decision)
        conditioning_blocks = {phase: ({"block": safety_message, "names": [], "why_log": [{"name": "blocked", "explanation": safety_message}], "grouped_drills": {}}) if context.phase_active(phase) else {} for phase in PHASES}
