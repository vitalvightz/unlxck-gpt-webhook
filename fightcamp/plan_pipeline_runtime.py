...
def build_runtime_context(
    *,
    ...
    def _normalize_goal_value(raw: str) -> str:
        value = str(raw or "").strip()
        if not value:
            return ""
        mapped = GOAL_NORMALIZER.get(value)
        if mapped is None:
            mapped = GOAL_NORMALIZER.get(value.lower())
        return str(mapped if mapped is not None else value).strip().lower()

    def _normalized_goals(raw_goals: str) -> list[str]:
        return [
            normalized
            for goal in (raw_goals or "").split(",")
            if (normalized := _normalize_goal_value(goal))
        ]
    ...
    def _normalize_weakness_values(raw: str) -> list[str]:
        value = str(raw or "").strip().lower()
        if not value:
            return []
        mapped = WEAKNESS_NORMALIZER.get(value)
        if mapped is None:
            return [value]
        if isinstance(mapped, list):
            return [str(token).strip().lower() for token in mapped if str(token).strip()]
        return [str(mapped).strip().lower()] if str(mapped).strip() else []

    def _resolve_primary_goal(normalized_goals: list[str], raw_primary: str) -> str:
        normalized_primary = _normalize_goal_value(raw_primary)
        if normalized_primary and normalized_primary in normalized_goals:
            return normalized_primary
        return normalized_goals[0] if normalized_goals else ""

    def _resolve_primary_weak_area(normalized_weaknesses: list[str], raw_primary: str) -> str:
        normalized_tokens = _normalize_weakness_values(raw_primary)
        for token in normalized_tokens:
            if token in normalized_weaknesses:
                return token
        return normalized_weaknesses[0] if normalized_weaknesses else ""
    ...
    primary_goal = _resolve_primary_goal(normalized_goals, plan_input.primary_goal)
    primary_weak_area = _resolve_primary_weak_area(normalized_weaknesses, plan_input.primary_weak_area)
