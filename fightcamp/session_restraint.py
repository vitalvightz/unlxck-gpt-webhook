...
def sort_weighted_candidates(
    ...
    *,
    ...
    Global score order stays primary. The list is first sorted by score descending with
    a stable alphabetical fallback, then only contiguous near-equal score groups are
    locally reordered by lower fatigue cost. This avoids the non-transitive pairwise
    comparator behavior that can happen when ranking logic switches per comparison.
    """
