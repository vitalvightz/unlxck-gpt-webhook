...
def _build_week_advisory(
    ...
    *,
    ...
    if action == "convert" and final_week_cap:
        suggestion = (
            f"If {future_state_label} {future_state_verb} still there by {week_label}, keep one hard sparring day and make {reported_days_label} technical only."
            if future_week
            else f"Keep one hard sparring day in {week_label}; make {reported_days_label} technical only."
        )
    elif action == "convert":
        suggestion = (
            f"If {future_state_label} {future_state_verb} still there by {week_label}, make hard sparring on {days_label} technical only."
            if future_week
            else f"Make hard sparring on {days_label} in {week_label} technical only."
        )
    elif final_week_cap:
        suggestion = (
            f"If {future_state_label} {future_state_verb} still there by {week_label}, keep one hard sparring day and lighten {reported_days_label}."
            if future_week
            else f"Keep one hard sparring day in {week_label}; lighten {reported_days_label}."
        )
    else:
        suggestion = (
            f"If {future_state_label} {future_state_verb} still there by {week_label}, lighten hard sparring on {days_label}."
            if future_week
            else f"Lighten hard sparring on {days_label} in {week_label}."
        )
