...
from .athlete_model import (
    ...
    is_high_pressure_weight_cut as _is_high_pressure_weight_cut,
    parse_record as _parse_record,
)
from .stage2_render_guards import (
    append_render_guard_writing_rules as _append_render_guard_writing_rules,
    render_guard_flags as _render_guard_flags,
    has_active_injury_from_training_context as _has_active_injury_from_training_context,
)
...
# Internal helper aliases for backward compatibility with local code
_slugify = slugify
...
def _build_finalizer_packet(payload: dict, *, days_until_fight: int | None = None) -> dict:
    """Slims the Stage 2 payload for the LLM finalizer.

    If in late-fight mode (countdown), we strip the candidate_pools and omission_ledger
    to save tokens and focus the LLM purely on the session-finalization task.
    """
    packet = dict(payload)
    if _uses_late_fight_stage2_payload(days_until_fight):
        packet.pop("candidate_pools", None)
        packet.pop("omission_ledger", None)
    return packet
...
def build_stage2_payload(
    *,
    ...
    if _uses_late_fight_stage2_payload(days_until_fight):
        ...
        return _build_finalizer_packet(payload, days_until_fight=days_until_fight)
