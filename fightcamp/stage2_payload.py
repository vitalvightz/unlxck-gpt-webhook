...
Every app-owned session must include exact drill/exercise, sets/reps/duration, rest, intensity or RPE, purpose, why today, and a progression/regression or stop rule. If a selected drill or exercise does not supply rest or intensity/RPE, create and state the missing value using a conservative dose appropriate to its exercise type, phase, fight proximity, and safety restrictions. Never exceed an authoritative RPE, volume, or rest constraint when one is supplied. From D-10 to the fight, that line must offer regressions and stop rules only — never a progression/advance option (no "add load/sets", "heavier ball", "stronger band", or "to progress"). Strength & conditioning sessions (strength, power, alactic, aerobic, fight-pace, neural speed work) lock earlier: from D-13 they too offer regressions and stop rules only. Fillers, rehab, mobility, and light recovery work may still progress on D-13 to D-11; D-14 and earlier may progress everywhere. Do not render generic slot labels such as "Strength", "Aerobic support", or "Low-load mobility support" without actual coaching content.
...
Do not expose internal role keys or internal system labels as session titles. Translate role keys into coach-voiced names from the intent, drills selected, and countdown day. Canonical mapping:
  ...
  hard_sparring_day          -> athlete-owned combat lock; subject to resolved safety, readiness, and calendar restrictions. Only for a resolved hard-as-planned day render minimally as "Hard sparring — controlled hard contact" or sport-equivalent (e.g. "MMA — hard sparring / controlled hard contact"). From D-14 normally, or D-17 with elevated risk, hard sparring is converted to technical work (also whenever the day carries reason code "d14_hard_sparring_ban" or "d17_hard_sparring_ban"): render "Technical-only combat" or sport-equivalent. The athlete owns the day (with or without a coach); the app must not prescribe rounds, intensity, dose, work:rest, RPE, or any sparring template wording, and no programmed S&C is scheduled on that day. After a hard-sparring label emit exactly one note: "Your declared hard-sparring/contact session — no extra S&C. Keep freshness priority." After a technical-only combat label emit exactly one note: "Technical-only contact today — no hard sparring and no extra S&C. Keep freshness priority." Nothing else.
...
5. Every app-owned training day must clearly show:
   ...
   - exact drill/exercise, sets/reps/duration, rest, and intensity/RPE. If a drill or exercise omits rest or intensity/RPE, create and state the missing value conservatively for the exercise type, phase, fight proximity, and restrictions; never exceed an authoritative supplied constraint.
