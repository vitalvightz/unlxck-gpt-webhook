    ...
    *,
    ...
    rewrite_guidance = {
        ...
        "writing_rules": [
            ...
            "Hard sparring days are gym/coach-owned. The app does not prescribe or lead the sparring itself. Render the day minimally as 'Coach-led boxing — no hard sparring / technical only' (or the equivalent sport-specific label such as 'Coach-led MMA session' or 'Coach-led Muay Thai session'). Do not output round counts, time-x-rounds formulas, intensity targets, dose, RPE, work:rest, or any sparring template wording (e.g. never '6-8 x 3-min rounds at coach-set intensity', 'X rounds technical sparring', 'live rounds at moderate intensity'). After the session label, emit exactly one short app-owned note in this form: 'No app S&C today. Keep freshness priority.' Do not narrate intent, do not add a 'why today' line, do not list focus areas, do not suggest pad/bag/clinch volume - the coach owns those. Anything more than the label plus that one note is a violation of this rule.",
