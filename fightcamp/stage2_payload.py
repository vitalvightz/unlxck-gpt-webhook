...
"Hard sparring days are gym/coach-owned. The app does not prescribe or lead the sparring itself. Default render is the minimal label '" + CANONICAL_HARD_SPARRING_LABEL + "' (or the equivalent sport-specific label such as 'Coach-led MMA session', 'Coach-led Muay Thai session', or 'Coach-led kickboxing session'). Hard sparring is not deloaded or downgraded by default — only deload when fatigue, weight cut, or another readiness flag explicitly demands it. When the day is D-17 or closer to the fight (or carries reason code 'd17_hard_sparring_ban'), override the label to '" + CANONICAL_HARD_SPARRING_BAN_LABEL + "' because hard sparring is banned in that window. Do not output round counts, time-x-rounds formulas, intensity targets, dose, RPE, work:rest, or any sparring template wording (e.g. never '6-8 x 3-min rounds at coach-set intensity', 'X rounds technical sparring', 'live rounds at moderate intensity'). After the session label, emit exactly one short note in this form: '" + CANONICAL_HARD_SPARRING_NOTE + "' Do not narrate intent, do not add a 'why today' line, do not list focus areas, do not suggest pad/bag/clinch volume - the coach owns those. Anything more than the label plus that one note is a violation of this rule.",
...
Never use the word "app" (or "the app", "this app", "the platform", "app-owned", "app-provided", "app S&C") in athlete-facing text. The athlete is reading their own plan — name the work directly ("your S&C and rehab inserts", "your programmed sessions", "no extra S&C today") and never attribute anything to an app.
...
Hard sparring days are gym/coach-owned. The app must not prescribe or lead the sparring itself. Default render is the minimal label "Coach-led boxing session" (or the equivalent sport-specific label such as "Coach-led MMA session", "Coach-led Muay Thai session", or "Coach-led kickboxing session"). Hard sparring is not deloaded by default — only deload when fatigue, weight cut, or another readiness flag explicitly demands it. When the day is D-17 or closer to the fight (or carries reason code "d17_hard_sparring_ban"), override the label to "Coach-led boxing — no hard sparring / technical only" because hard sparring is banned in that window. Do not output round counts, time-x-rounds formulas, intensity targets, dose, RPE, work:rest, or any sparring template wording. After the session label, emit exactly one short note in this form: "No extra S&C today. Keep freshness priority." Do not narrate intent, do not add a "why today" line, do not list focus areas, do not suggest pad/bag/clinch volume — the coach owns those. Anything more than the label plus that one note is a violation of this rule.
...
Do not expose internal role keys or internal system labels as session titles. Translate role keys into coach-voiced names from the intent, drills selected, and countdown day. Canonical mapping:
  ...
  hard_sparring_day          -> default render minimally as "Coach-led boxing session" or sport-equivalent: "Coach-led MMA session", "Coach-led Muay Thai session", "Coach-led kickboxing session". Hard sparring is not deloaded by default — only deload when fatigue, weight cut, or another readiness flag explicitly demands it. When the day is D-17 or closer to the fight (or carries reason code "d17_hard_sparring_ban"), override the label to "Coach-led boxing — no hard sparring / technical only" because hard sparring is banned in that window. The gym/coach owns the day; the app must not prescribe rounds, intensity, dose, work:rest, RPE, or any sparring template wording. After the label, emit exactly one short note: "No extra S&C today. Keep freshness priority." Nothing else.
