...
def _handoff_mode_instructions(payload_mode: str) -> str:
    _CONTRACT = (
        ...
        "Hard sparring days are gym/coach-owned. The app must not prescribe the sparring. Render the day as a minimal label only — \"Coach-led boxing — no hard sparring / technical only\" (or sport-equivalent). No round counts, no time-x-rounds, no intensity targets, no dose, no RPE, no work:rest, no sparring template wording. After the label, emit exactly one app-owned note: \"No app S&C today. Keep freshness priority.\" Nothing else."
    )
