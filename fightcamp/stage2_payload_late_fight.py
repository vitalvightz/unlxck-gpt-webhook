...
def _full_countdown_weekday_map(days_until_fight: Any, athlete_model: dict[str, Any]) -> dict[str, str]:
    ...
    return _resolve_countdown_weekday_with_availability(
        countdown_map,
...
def _score_composite_practical_assignment(
    ...
    for role in assigned_roles:
        ...
        if _meaningful_app_owned_role(role):
...
def _build_late_fight_context_overlay(
    days_until_fight: Any,
    athlete_model: dict[str, Any],
    phase_briefs: dict[str, dict],
    days_out_payload: dict[str, Any],
    late_fight_plan_spec: dict[str, Any],
    late_fight_session_sequence: list[dict[str, Any]],
) -> dict[str, Any]:
    phase = _resolve_late_fight_phase(phase_briefs)
    phase_brief = dict((phase_briefs or {}).get(phase) or {})
    objective = str(phase_brief.get("objective") or "").strip()
    emphasize = [str(item).strip() for item in clean_list(phase_brief.get("emphasize", []))]
    protect = [str(item).strip() for item in clean_list(phase_brief.get("protect", []))]
    deprioritize = [str(item).strip() for item in clean_list(phase_brief.get("deprioritize", []))]

    arc_defaults = {
        "d21_to_d14": "Bridge: preserve useful qualities while reducing fatigue risk.",
        "d13_to_d8": "Compressed pre-fight week: maintain sharpness and remove development work.",
        "d7": "Sharpness week entry: keep one low-volume power touch.",
        "d6_to_d5": "Transition window: freshness priority and no density stacking.",
        "d4_to_d2": "Final sharpening: low-noise neural cue only.",
        "d1": "Primer day: rhythm only and no loading.",
        "d0": "Fight-day protocol only.",
    }
    late_stage_arc: dict[str, str] = {}
    for segment in (late_fight_plan_spec.get("countdown_mode_sequence") or []):
        if not isinstance(segment, dict):
            continue
        stage_key = str(segment.get("stage_key") or "").strip()
        if stage_key and stage_key in arc_defaults:
            late_stage_arc[stage_key] = arc_defaults[stage_key]
    if not late_stage_arc:
        fallback_stage = str(days_out_payload.get("late_fight_window") or "").strip()
        if fallback_stage and fallback_stage in arc_defaults:
            late_stage_arc[fallback_stage] = arc_defaults[fallback_stage]

    purpose_by_role = {
        "strength_touch_day": "Maintain neural power without creating soreness.",
        "fight_week_freshness_day": "Restore freshness and preserve tissue quality.",
        "neural_primer_day": "Sharpen fight-speed timing without fatigue.",
        "technical_touch_day": "Keep technical rhythm and neural coordination without density.",
        "light_fight_pace_touch_day": "Keep fight-specific timing with low fatigue cost.",
        "alactic_sharpness_day": "Prime speed and intent with full recovery between reps.",
    }
    session_purpose_by_day: dict[str, str] = {}
    for entry in late_fight_session_sequence or []:
        if not isinstance(entry, dict):
            continue
        offset = entry.get("countdown_offset")
        role_key = str(entry.get("role_key") or "").strip()
        if not isinstance(offset, int) or offset < 0 or not role_key:
            continue
        purpose = purpose_by_role.get(role_key)
        if purpose:
            session_purpose_by_day[f"D-{offset}"] = purpose

    overlay: dict[str, Any] = {
        "original_phase_context": {
            "phase": phase,
            "objective": objective,
            "emphasize": emphasize,
            "protect": protect,
        ...
        "late_stage_arc": late_stage_arc,
        "session_purpose_by_day": session_purpose_by_day,
        "coach_day_policy": {
            "coach_led_days": "no app S&C",
            "allowed": "optional recovery-only support if clearly labelled non-S&C",
        },
        "writing_upgrade": {
            "avoid_terms": [
                "survival",
                "primary strength option for technical tempo",
                "phase-critical drills preserved",
                "active notes",
            ],
            "prefer_terms": [
                "final readiness",
    ...
    has_active_injury = (
        bool(athlete_model.get("has_active_injury"))
        or bool(clean_list(athlete_model.get("injuries", [])))
        or bool(clean_list(athlete_model.get("restrictions", [])))
        or "injury_management" in readiness_flags
    )
    if has_active_injury:
        overlay["injury_support_policy"] = {
            "active_injury": True,
            "allowed_support": ["mobility", "band reset", "low-load tissue care"],
            "blocked": ["heavy elbow flexion", "long holds", "fatiguing upper-limb work"],
            "stop_rules": [
                "sharp pain",
                "sudden weakness",
                "new numbness or tingling",
                "marked next-morning worsening",
            ],
        }

    raw_weight_cut_pct = athlete_model.get("weight_cut_pct")
    try:
        weight_cut_pct_float = float(raw_weight_cut_pct or 0.0)
    except (TypeError, ValueError):
        weight_cut_pct_float = 0.0

    cut_bucket = str(athlete_model.get("cut_severity_bucket") or "").strip().lower()
    has_meaningful_cut = (
        weight_cut_pct_float >= 3.0
        or cut_bucket in {"moderate", "high", "critical", "extreme"}
        or bool(athlete_model.get("weight_cut_risk"))
    )
    if has_meaningful_cut:
        overlay["weight_cut_policy"] = {
            "cut_pct": raw_weight_cut_pct,
            "severity": cut_bucket or "moderate",
            "rule": "reduce accessory volume and keep density low",
        }
