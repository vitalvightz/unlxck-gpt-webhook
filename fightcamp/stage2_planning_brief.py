...
from .stage2_restriction_utils import (  # noqa: F401  (re-exported for backwards compatibility)
    ...
    _derive_mechanical_risk_tags,
    _extract_mechanical_risk_tags,
    _extract_restriction_tags,
    _restriction_item_text,
    _restriction_patterns_for_key,
...
def _compress_short_camp_priorities(athlete_model: dict) -> dict:
    ...
    if speed_signal and footwork_signal:
        ...
        speed_bucket = (
            ...
            if (
                speed_goal_signal
                or footwork_goal_signal
                or primary_goal_tokens & (_SPEED_SHARPNESS_TOKENS | _FOOTWORK_QUALITY_TOKENS)
            )
            else embedded
        )
        add_unique(
