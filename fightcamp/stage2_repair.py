...
18. Treat declared hard sparring days in weekly_role_map as immutable hard_sparring_day slots. Hard sparring days are gym/coach-owned combat locks: the app does not prescribe or lead the sparring itself, and it never deloads, caps, or drops a declared hard sparring day. At D-18 or further out render the minimal label "Coach-led boxing — hard sparring / controlled hard contact" (or the equivalent sport-specific label such as "Coach-led MMA — hard sparring / controlled hard contact"). From D-17 onward hard sparring is banned: render "Coach-led boxing — technical-only combat" (or sport-equivalent) — the same applies whenever the day carries reason code "d17_hard_sparring_ban". Do not output round counts, time-x-rounds formulas, intensity targets, dose, RPE, work:rest, or any sparring template wording (e.g. never "6-8 x 3-min rounds at coach-set intensity", "X rounds technical sparring", "live rounds at moderate intensity"). After the label, emit exactly one short note: "Coach-owned combat session. Keep freshness priority." Nothing else — no programmed S&C is scheduled on a coach-owned combat day. If the previous plan rendered rounds, intensity, dose, or template sparring detail, strip it down to this minimal form.
