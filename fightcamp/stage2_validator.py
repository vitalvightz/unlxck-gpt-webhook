...
def validate_stage2_output(*, planning_brief: dict, final_plan_text: str) -> dict:
    ...
    for internal in ("role_key", "candidate_pool", "planning_brief", "stage2_payload", "stage2_handoff", "validator_report", "why_log", "taper_micro_support", "render_contract"):
        if re.search(rf"\b{re.escape(internal)}\b", text_lower):
            errors.append(_issue(code="true_internal_system_leak", message=f"Internal system term leaked: {internal}.", severity="blocker", confidence="high", line=internal))
    if re.search(r"\bD-(?:1[0-6]|[0-9])\b.*\b(hard spar|hard sparring|live spar|full spar|hard contact)\b", final_plan_text, re.IGNORECASE):
        errors.append(_issue(code="late_fight_hard_sparring_violation", message="Hard sparring appears inside D-16 to D-0.", severity="blocker", confidence="high"))
    if re.search(r"\bD-1\b.*\b(strength|conditioning|sprints?|interval|heavy|loaded|deadlift|squat|trap bar|barbell)\b", final_plan_text, re.IGNORECASE):
        errors.append(_issue(code="dangerous_late_fight_strength_or_conditioning", message="D-1 includes hard strength/conditioning exposure.", severity="blocker", confidence="medium"))
    if re.search(r"\bD-0\b.*\b(?:extra|plus|add|conditioning|strength|finisher|circuit|sprint|lift)\b", final_plan_text, re.IGNORECASE):
        errors.append(_issue(code="fight_day_protocol_violation", message="D-0 includes extra training beyond fight-day protocol.", severity="blocker", confidence="medium"))
