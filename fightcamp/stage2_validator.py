...
_LIGHT_TECHNICAL_PATTERN = re.compile(
    r"\b(" + "|".join(re.escape(term) for term in LIGHT_TECHNICAL_TERMS) + r")\b",
    re.IGNORECASE,
)
_LIGHT_TECHNICAL_MODIFIER_PATTERN = re.compile(r"\b(light|technical|flow|touch|controlled|drill)\b", re.IGNORECASE)


def _has_blocking_hard_sparring(text: str) -> bool:
    lowered = str(text or "")
    has_hard_sparring = _HARD_SPARRING_PATTERN.search(lowered) is not None
    if not has_hard_sparring:
        return False
    has_light_technical_modifier = (
        _LIGHT_TECHNICAL_PATTERN.search(lowered) is not None
        or ("sparring" in lowered.lower() and _LIGHT_TECHNICAL_MODIFIER_PATTERN.search(lowered) is not None)
    )
    return has_hard_sparring and not has_light_technical_modifier
...
def validate_stage2_output(*, planning_brief: dict, final_plan_text: str) -> dict:
    ...
    for block in countdown_blocks:
        ...
        if day == 12 and _has_blocking_hard_sparring(joined):
        ...
        if 0 <= day <= 11 and _has_blocking_hard_sparring(joined):
