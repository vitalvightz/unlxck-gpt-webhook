...
def generate_strength_block(*, flags: dict, weaknesses=None, mindset_cue=None):
    ...
    # recovery cost (soreness, neural complexity, aggravation risk).  This
    # replaces the plain score-only sort so no separate presort is needed.
    _tie_break_sort(weighted_exercises)
    ...
    # its purpose without it.
    base_exercises, _pruned = _pruning_pass(
        base_exercises,
        phase=phase,
        fatigue=fatigue,
        days_until_fight=flags.get("days_until_fight"),
    ...
    if _pruned:
        logger.info(
            "[restraint] pruned phase=%s fatigue=%s name=%s score=%.4f",
            phase,
            fatigue,
        ...
        why_log.append(
            {
                ...
                "explanation": "removed by final pruning pass (taper/short-notice/high-fatigue context)",
            }
        )
