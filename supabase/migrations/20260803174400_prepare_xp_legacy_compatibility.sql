-- Install the compatibility layer before any stricter XP constraint or trigger.
-- The currently deployed backend omits p_calendar_date for calendar-scoped XP.
-- Derive it only from persisted authoritative records so every migration prefix
-- remains compatible while the database is deployed ahead of the new backend.
