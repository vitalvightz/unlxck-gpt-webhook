-- Blank global feedback is not a rewardable action. Contextual feedback remains
-- eligible because its persisted response/reason is itself useful product data.
-- Remove the invalid awards already created by the blank-submit exploit while
-- keeping xp_accounts consistent with the ledger.

with invalid_totals as (
  select
    award.athlete_id,
    sum(award.amount)::bigint as xp_to_remove
  from public.xp_awards award
  join public.beta_feedback feedback
    on award.idempotency_key = 'feedback:' || feedback.id::text
  where feedback.surface = 'global'
    and regexp_replace(coalesce(feedback.comment, ''), '[[:space:]]+', '', 'g') = ''
    and nullif(btrim(feedback.screenshot_path), '') is null
  group by award.athlete_id
)
update public.xp_accounts account
set
  total_xp = greatest(0, account.total_xp - invalid_totals.xp_to_remove),
  updated_at = clock_timestamp()
from invalid_totals
where account.athlete_id = invalid_totals.athlete_id;

delete from public.xp_awards award
using public.beta_feedback feedback
where award.idempotency_key = 'feedback:' || feedback.id::text
  and feedback.surface = 'global'
  and regexp_replace(coalesce(feedback.comment, ''), '[[:space:]]+', '', 'g') = ''
  and nullif(btrim(feedback.screenshot_path), '') is null;
...
declare
  ...
  v_feedback_comment text;
  v_feedback_has_screenshot boolean := false;
  v_feedback_eligible boolean := true;
...
begin
  ...
  select
    ...
    surface::text,
    comment,
    nullif(btrim(screenshot_path), '') is not null
  into
    v_feedback_created_at,
    v_feedback_surface,
    v_feedback_comment,
    v_feedback_has_screenshot
  ...
  -- A global category click alone is not feedback. A written description or a
  -- persisted screenshot is required. Contextual plan/Today feedback remains
  -- eligible because its response is already meaningful structured input.
  v_feedback_eligible :=
    v_feedback_surface <> 'global'
    or regexp_replace(coalesce(v_feedback_comment, ''), '[[:space:]]+', '', 'g') <> ''
    or v_feedback_has_screenshot;
