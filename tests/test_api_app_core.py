...
def test_production_cors_rejects_unsafe_origin_by_default(monkeypatch: pytest.MonkeyPatch):
    _clear_env_detection_vars(monkeypatch)
    monkeypatch.setenv("APP_ENV", "production")
...
def test_production_cors_explicit_unsafe_override_allows_boot(
    ...
    assert "production_cors_unsafe" in combined
    assert "APP_ALLOW_UNSAFE_PRODUCTION_CORS_BOOT=1" in combined
