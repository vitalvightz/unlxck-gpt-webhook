...
class TestRollover:
    def test_0359_local_maps_to_previous_training_day(self):
        # 03:59 local is before the 04:00 rollover -> previous training day.
        ts = datetime(2026, 6, 18, 3, 59, tzinfo=ZoneInfo(NY))
        assert resolve_training_day(ts, athlete_timezone=NY) == date(2026, 6, 17)

    def test_0400_local_maps_to_current_training_day(self):
        # 04:00 local is exactly the rollover -> current training day.
        ts = datetime(2026, 6, 18, 4, 0, tzinfo=ZoneInfo(NY))
...
class TestHelpers:
    def test_rollover_hour_constant_is_four(self):
        assert DAY_ROLLOVER_HOUR == 4
