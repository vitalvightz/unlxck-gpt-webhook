...
- hard-sparring planner converting D-17+ days to technical and capping
  D-21..D-18 to a single effective hard exposure,
...
def test_d17_hard_sparring_converts_to_technical():
    ...
    # Wednesday is D-16: must be converted (banned).
    assert by_day["Wednesday"]["status"] == "convert_to_technical_suggested"
    assert by_day["Wednesday"]["effective_load"] == "technical"
    assert "d17_hard_sparring_ban" in by_day["Wednesday"]["reason_codes"]

    # Monday is D-18 and Friday is D-14 (also banned).
    assert by_day["Friday"]["effective_load"] == "technical"
    assert "d17_hard_sparring_ban" in by_day["Friday"]["reason_codes"]


def test_d21_d18_window_caps_to_one_effective_hard_day():
    # Week ends at D-18, span 4, so days are D-18..D-21 — fully inside the cap window.
    # Friday fight → D-18 = Monday, D-19 = Sunday, D-20 = Saturday, D-21 = Friday.
    week = _week_with_calendar(
    ...
    plan = compute_hard_sparring_plan(
        week=week,
        athlete_snapshot=_athlete(22, hard_days=["Friday", "Monday"]),
    )
...
def test_normal_camp_4_week_emits_calendar_metadata_and_bans_d17():
