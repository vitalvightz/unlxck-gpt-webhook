...
def test_surface_only_dict_injury_type_still_uses_surface_bypass():
    decision = injury_decision(
        {"name": "Push Press", "tags": ["upper_push", "neck_loaded"]},
        [{"injury_type": "cut", "region": "neck", "severity": "mild"}],
        "GPP",
        "low",
    )
    assert decision.action == "allow"
...
def test_surface_plus_ambiguous_non_surface_phrase_does_not_bypass():
    decision = injury_decision(
        {"name": "Depth Jump", "tags": ["landing_stress_high", "high_impact_plyo"]},
        ["neck cut", "knee issue"],
        "GPP",
