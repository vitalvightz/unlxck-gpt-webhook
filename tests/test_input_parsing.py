...
def test_guided_serious_type_kept_when_parser_type_absent():
    payload = _payload(
        [
            {"label": "Full name", "value": "Test Athlete"},
            {"label": "Fighting Style (Technical)", "value": "Boxing"},
    ...
    payload["guided_injury"] = {"area": "right hand", "injury_type": "fracture"}
    parsed = PlanInput.from_payload(payload)
    assert parsed.parsed_injuries[0]["injury_type"] == "fracture"
    assert parsed.parsed_injuries[0]["canonical_location"] == "hand"

    payload["guided_injury"] = {"area": "right shoulder", "injury_type": "dislocation"}
    parsed = PlanInput.from_payload(payload)
    assert parsed.parsed_injuries[0]["injury_type"] == "dislocation"
    assert parsed.parsed_injuries[0]["canonical_location"] == "shoulder"


def test_guided_tendon_ligament_without_rupture_evidence_does_not_escalate():
    payload = _payload(
        [
            {"label": "Full name", "value": "Test Athlete"},
            {"label": "Fighting Style (Technical)", "value": "Boxing"},
    ...
    payload["guided_injury"] = {"area": "right knee", "injury_type": "tendon_ligament"}
    parsed = PlanInput.from_payload(payload)
    assert parsed.parsed_injuries[0]["injury_type"] != "tendon_rupture_or_avulsion"


def test_guided_surface_injury_with_bruise_maps_to_contusion():
    payload = _payload(
        [
            {"label": "Full name", "value": "Test Athlete"},
            {"label": "Fighting Style (Technical)", "value": "Boxing"},
...
def test_parser_specific_type_wins_over_structured_fracture():
    payload = _payload(
        [
            {"label": "Full name", "value": "Test Athlete"},
            {"label": "Fighting Style (Technical)", "value": "Boxing"},
    ...
    parsed = PlanInput.from_payload(payload)
    assert parsed.parsed_injuries[0]["injury_type"] == "hyperextension"
