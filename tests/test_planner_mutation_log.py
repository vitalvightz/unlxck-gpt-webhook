...
@pytest.mark.parametrize("fight_format", ["boxing", "mma"])
def test_unilateral_selection_survives_naturally_in_balance_cases(monkeypatch, fight_format):
    """Unilateral work should still surface naturally from scoring signals.

    This protects the watch-item from review feedback: even after narrowing
    promote_base_categories, unilateral should remain available in realistic
    boxing/MMA balance-footwork contexts without requiring post-score
    compensation.
    """
    bank = [
        {
            "name": "Split Squat",
            "phases": ["GPP"],
            "tags": ["Split Squat", "unilateral", "compound", "balance", "footwork"],
            "equipment": ["dumbbell"],
            "movement": "squat",
        },
        {
            "name": "Back Squat",
            "phases": ["GPP"],
            "tags": ["Back Squat", "compound", "quad_dominant"],
            "equipment": ["barbell"],
            "movement": "squat",
        },
        {
            "name": "Bench Press",
            "phases": ["GPP"],
            "tags": ["Bench Press", "compound", "push"],
            "equipment": ["barbell"],
            "movement": "push",
        },
    ]
    ...
    def _fake_score(exercise_tags, **_kwargs):
        # Simulate the scorer strongly preferring unilateral in a balance/footwork
        # context so selection should happen naturally, not via compensation.
        scores = {"Split Squat": 9.0, "Back Squat": 6.0, "Bench Press": 5.5}
        tag = exercise_tags[0] if exercise_tags else ""
        v = scores.get(tag, 1.0)
        return v, {"final_score": v}
    ...
    result = strength_module.generate_strength_block(
        flags={
            **_base_flags("GPP"),
            ...
            "weaknesses": ["balance", "footwork", "coordination"],
        }
    )

    exercise_names = {ex.get("name") for ex in result["exercises"]}
    assert "Split Squat" in exercise_names, (
        "Expected scorer-led unilateral selection to survive in balance/footwork context; "
        f"got exercises={exercise_names}"
    )
    unilateral_comp_entries = [
        e
        for e in result["mutation_log"]
        if e["mechanism"] == "promote_base_categories" and "unilateral" in e["reason"]
    ]
    assert unilateral_comp_entries == [], (
        "Unilateral should be scorer-led here, not inserted via selector compensation; "
        f"mutation_log={result['mutation_log']}"
    )
