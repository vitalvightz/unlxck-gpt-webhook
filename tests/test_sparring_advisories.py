...
def test_returns_only_one_best_advisory_when_multiple_weeks_qualify():
    ...
    assert advisories[0]["suggestion"].startswith("If high fatigue, an aggressive cut, and worsening ankle instability are still there by d2")
...
def test_future_week_advisory_uses_conditional_static_app_wording():
    ...
    assert "worsening ankle instability" in advisory["reason"]
    assert advisory["suggestion"].startswith("If high fatigue, an active cut, and worsening ankle instability are still there by d2")
