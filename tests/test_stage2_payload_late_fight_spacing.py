...
def test_freshness_role_is_structural_not_just_first_free_slot():
    ...
    active_offsets = _offsets(sequence)
    prior_offsets = [offset for offset in active_offsets if offset > freshness_offset]
    later_offsets = [offset for offset in active_offsets if offset < freshness_offset]

    gap_before = (min(prior_offsets) - freshness_offset) if prior_offsets else None
    gap_after = (freshness_offset - max(later_offsets)) if later_offsets else None

    # Freshness should either break density with a meaningful gap, or sit near
    # the end as part of an intentional taper shape.
    breaks_density = (
        gap_before is not None
        and gap_after is not None
        and (gap_before >= 2 or gap_after >= 2)
    )
    taper_shape = freshness_offset <= 2 and gap_before is not None and gap_before >= 2
    assert breaks_density or taper_shape
