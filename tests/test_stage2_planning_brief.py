...
def test_fight_week_override_day_zero_outputs_activation_protocol_only_and_no_week_roles():
    brief = _build_progression_brief(
        {
            "sport": "boxing",
            "status": "amateur",
    ...
    assert brief["fight_week_override"]["band"] == "fight_day_activation_protocol"
    assert brief["weekly_role_map"]["fight_week_override"]["band"] == "fight_day_activation_protocol"
    assert brief["weekly_role_map"]["weeks"] == []


def test_fight_week_override_day_before_keeps_single_primer_week():
    brief = _build_progression_brief(
        {
            "sport": "boxing",
            "status": "amateur",
    ...
    assert brief["fight_week_override"]["active"] is True
    assert brief["fight_week_override"]["band"] == "day_before_primer_protocol"
    week = brief["weekly_role_map"]["weeks"][0]
    assert len(week["session_roles"]) == 1
    assert week["session_roles"][0]["role_key"] in {"alactic_sharpness_day", "fight_week_freshness_day"}
