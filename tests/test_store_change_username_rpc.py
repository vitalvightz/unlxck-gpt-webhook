...
def test_change_username_rate_limit_parsing_ignores_structured_suffix():
    store = _make_store()
    store._require_profile = MagicMock(return_value=_profile())
    store.client.rpc.return_value.execute.side_effect = PostgrestAPIError(
        {
            "message": "username_rate_limit_exceeded:2026-06-01T00:00:00+00:00",
            "details": {"context": "ignored"},
        }
    )
    ...
    assert exc_info.value.status_code == status.HTTP_429_TOO_MANY_REQUESTS
    assert "Next change available 2026-06-01T00:00:00+00:00." in exc_info.value.detail
    assert "{'context': 'ignored'}" not in exc_info.value.detail
