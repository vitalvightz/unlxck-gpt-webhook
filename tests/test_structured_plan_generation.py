...
def test_normalize_effort_accepts_scalars_ranges_and_textual_cues():
    ...
    for value in ("RPE 6-7", "RPE 6–7"):
        assert _normalize_effort(value) == {
            "method": "RPE",
            "value": "6-7",
            "scale": "1-10",
        }
    ...
    assert _normalize_effort(
        {"method": "intent", "value": "fast but relaxed", "scale": None}
    ) == {"method": "intent", "value": "fast but relaxed", "scale": None}


def test_normalize_effort_rejects_reversed_and_malformed_numeric_ranges():
    for value in ("RPE 8-6", "RIR 4-2", "RPE 7--8", "RPE 7-", "RPE -8"):
        assert _normalize_effort(value) is None
    assert _normalize_effort(
...
def test_invalid_optional_effort_is_dropped_without_rejecting_card():
    plan = _valid_plan()
    block = plan["weeks"][0]["days"][0]["sessions"][0]["blocks"][0]
    block["effort"] = {"method": "RPE", "value": "8-6", "scale": "1-10"}
