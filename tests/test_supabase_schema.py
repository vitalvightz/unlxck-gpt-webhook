...
def test_schema_includes_plan_generation_short_window_rate_limit_objects():
    ...
    assert "athlete_id uuid not null references public.profiles(id) on delete cascade" in schema
    ...
    assert "p_athlete_id uuid" in schema
...
def test_plan_generation_short_window_migration_uses_advisory_lock():
    ...
    assert "hashtext('plan_generation_rate_limit:' || p_athlete_id::text)" in migration
    assert "where athlete_id = p_athlete_id" in migration
