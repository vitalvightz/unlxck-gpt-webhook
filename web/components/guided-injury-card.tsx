...
function handleTypeSelect(opt: InjuryTypeOption | null) {
  ...
  function handleFamilySelect(family: InjuryFamily) {
    ...
    if (currentFamily && currentFamily !== family) {
      ...
      if (family === "pain_movement") handleTypeSelect({ label: "Pain / soreness", value: "pain" });
      if (family === "structural") handleTypeSelect({ label: "Fracture / broken bone", value: "fracture" });
      if (family === "head_nerve_breathing") handleTypeSelect({ label: "Head impact / concussion", value: "head_impact" });
      if (family === "surface") handleTypeSelect({ label: "Cut / wound", value: "surface_injury", surface_type: "cut" });
      if (family === "not_sure") handleTypeSelect({ label: "Not sure", value: "unspecified" });
      if (stripPrefixes.length) onUpdate("notes", stripTaggedNotes(injury.notes, stripPrefixes));
      return;
    }
    if (!currentFamily || currentFamily !== family) {
