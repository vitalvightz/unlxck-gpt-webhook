...
export function PublicGenerationScreen({ phase, error = null, failureKind = null, milestones = [], startedAtMs = null, canRetry = false, onRetry = null, onReturnToWorkspace = null, onRefineIntake = null }: Props) {
  ...
  const progress = getPublicProgress(phase, milestones, startedAtMs, now);
  ...
  return (
    <section className="public-build">
      ...
      <ol className="public-build-milestones">
        {PUBLIC_CAMP_MILESTONES.map((item, index) => { const state = index < activeIndex || phase === "finalizing" ? "complete" : index === activeIndex ? "active" : "future"; return <li key={item.code} className={`public-build-milestone public-build-milestone-${state}`}><span>{state === "complete" ? "✓" : ""}</span><div><strong>{item.title}</strong><small>{item.detail}</small></div></li>; })}
      </ol>
