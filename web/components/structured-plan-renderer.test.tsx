...
test("renders coach-owned day title above optional support blocks", () => {
  const plan = {
    schema_version: "1.0",
    plan_metadata: { title: "Fight Camp", sport: "boxing", plan_type: "fight_camp" },
    weeks: [
            ...
            optional_support_blocks: [
              {
                ...
                objective: "Optional low-noise support under the coach card",
                blocks: [],
                app_prescription: "optional_support_only",
                render_as_secondary_addon: true,
                parent_role_key: "hard_sparring_day",
              },
            ],
          },
        ],
  ...
  assert.equal(html.includes("Coach-led boxing — technical only"), true);
  assert.equal(html.includes("Optional support"), true);
  assert.equal(html.includes("Mobility, breathing &amp; lateral reset"), true);
  assert.equal(html.indexOf("Coach-led boxing"), html.indexOf("Coach-led boxing — technical only"));
  assert.equal(html.indexOf("Coach-led boxing") < html.indexOf("Mobility, breathing"), true);
});

test("renders light combat as coach-owned primary with optional footwork support", () => {
  const plan = {
    schema_version: "1.0",
    plan_metadata: { title: "Fight Camp", sport: "boxing", plan_type: "fight_camp" },
    weeks: [
        ...
        days: [
          {
            ...
            optional_support_blocks: [
              {
                session_id: "footwork-1",
                session_type: "skill",
                title: "Optional footwork focus",
                objective: "Step-back pivot reset only if the coach wants it",
                blocks: [],
                app_prescription: "optional_support_only",
                render_as_secondary_addon: true,
                parent_role_key: "light_combat_day",
              },
            ],
          },
        ],
  ...
  assert.equal(html.includes("Light technical combat tag"), true);
  assert.equal(html.includes("Optional support"), true);
  assert.equal(html.includes("Optional footwork focus"), true);
  assert.equal(html.includes("0 sessions"), false);
});
