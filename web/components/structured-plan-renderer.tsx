...
// Display-only label formatter (see lib/plan-labels). Kept as a short alias so
// the JSX stays readable; never mutates stored enum values.
const titleize = formatPlanLabel;

/**
 * A native <details> collapsible whose open state survives parent re-renders.
 * Used for dense sections (nutrition/recovery phases) so non-current phases can
 * default closed while staying one tap away. All content stays in the DOM.
 */
  ...
  return (
    ...
    >
      <summary className="sp-week-summary">
        ...
        {phase ? <span className="sp-tag sp-accent">{titleize(phase)}</span> : null}
      </summary>
    ...
    <section className="sp-card sp-nutrition">
      ...
      {deterministic ? (
        getDeterministicNutritionPhases(plan)
          ...
          // First (nearest) phase open by default; later phases collapsed but
          // one tap away. No nutrition data is removed.
          .map((item, index) => (
            <CollapsibleSection
              key={item.phase}
  ...
  return (
    <section className="sp-card sp-recovery">
      ...
      {getDeterministicRecoveryPhases(plan)
        ...
        .filter((item): item is NonNullable<typeof item> => item !== null)
        // First (nearest) phase open by default; later phases collapsed.
        .map((item, index) => (
          <CollapsibleSection
            key={item.phase}
            title={titleize(item.phase)}
