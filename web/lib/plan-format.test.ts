...
test("uses only finite week numbers for open-plan labels", () => {
  assert.equal(resolveFiniteWeekNumber(Number.NaN, 3), 3);
  assert.equal(resolveFiniteWeekNumber(Number.POSITIVE_INFINITY, Number.NaN), 1);
  assert.equal(resolveFiniteWeekNumber(null, undefined, 2), 2);
});
