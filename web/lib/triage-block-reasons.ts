...
function selectGuidedInjuryContext(
  ...
  const scored = guidedInjuries
    .map((injury, index) => {
      ...
      let score = 0;
      const processedForSubstring = new Set<string>();
      for (const field of fields) {
        if (!processedForSubstring.has(field.value)) {
          if (signals.some((signal) => signal.includes(field.value) || field.value.includes(signal))) {
            score += 5;
          }
          processedForSubstring.add(field.value);
        }
